const Filters = ( {filterCategory}  ) => {
    ...
    return(
        <div className="bg-emerald-50 m-8 text-stone-600 border border-transparent drop-shadow-xl rounded-2xl p-4 flex justify-center items-center flex-col md:flex-row">
            ...
            <select className=" text-stone-600 border border-transparent drop-shadow-xl rounded-2xl text-center py-1">
            {categorys.map(category => (
                <option onClick={() => filterCategory(category)} key={category}>{category}</option>
            ))}
            </select>
        </div>
    )
